...
const handleSearch = (e) => {
	if (searchText.value.trim()) {
		window.location.href = `https://imsuk.cn/search/${encodeURIComponent(searchText.value.trim())}`;
	}
	e.preventDefault();
};
...
<template>
	<div class="iro-root" :class="{ 'iro-light': !iroDark, 'iro-dark': iroDark }">
		<form class="iro-nav-search" :class="{ 'iro-show-search': iroShowSearch }" @submit="handleSearch" role="search">
			<div class="iro-search-form">
				<div class="micro">
					<p class="micro mb-">想要找点什么呢？</p>
					<input class="text-input" type="search" v-model="searchText" placeholder="搜索"
						required="">
				</div>
			</div>
